...
void writeInTransactionFile(const Transaction& tran) {
	ofstream file(transFileName, ios::binary | ios::app);
	if (!file.good()) {
		cerr << transFileName << " Writing in file failed!" << endl;
		return;
	}
...
bool readTransationByRecID(Transaction& tran) {
	ifstream file(transFileName, ios::binary);
	if (!file.good()) {
		cerr << transFileName << " Reading file failed!" << endl;
		return false;
	}
	Transaction temp;
	while (!file.eof()) {
...
bool readTransationBySendID(Transaction& tran) {
	ifstream file(transFileName, ios::binary);
	if (!file.good()) {
		cerr << transFileName << " Reading file failed!" << endl;
		return false;
...
void createTransaction(const unsigned int senderId, const unsigned int receiverId, const double fmiCoins) {
	Transaction tran;
	tran.senderId = senderId;
	tran.receiverId = receiverId;
	tran.fmiCoins = fmiCoins;
	tran.time = (long long)(time(NULL));
	writeInTransactionFile(tran);
}